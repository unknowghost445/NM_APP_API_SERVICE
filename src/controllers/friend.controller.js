...
exports.sendFriendRequest = async (req, res) => {
    try {
        ...
        const { data: existingFriendship } = await supabase
            .from('friends')
            .select('*')
            .or(`and(user_a_id.eq.${fromUserId},user_b_id.eq.${toUserId}),and(user_a_id.eq.${toUserId},user_b_id.eq.${
fromUserId})`)
            .maybeSingle();
        ...
        const { data: existingRequest } = await supabase
            ...
            .select('*')
            .eq('status', 'pending')
            .or(`and(from_user_id.eq.${fromUserId},to_user_id.eq.${toUserId}),and(from_user_id.eq.${toUserId},to_user_
id.eq.${fromUserId})`)
            .maybeSingle();

        if (existingRequest) {
            if (existingRequest.from_user_id === toUserId && existingRequest.to_user_id === fromUserId) {
                await supabase
                    .from('friend_requests')
